...
const ThenAction = ({id}: Props) => {
  const {isPending,listener:Listener,onFormSubmit,onSetListener,register} = useListener(id)
  return (
   <TriggerButton lable='Then'>
    <div className='flex flex-col gap-y-2'>
      {
        AUTOMATION_LISTENERS.map((listener)=>
        listener.type === 'SMARTAI' ? 
        (<SubscriptionPlan key={listener.type} 
        type='PRO'>
      ...
      <form onSubmit={onFormSubmit} className='flex flex-col gap-y-2'>
        ...
        />
          ...
          {...register('reply')}
          placeholder=
            'Add an reply for comments (Optional)'
            className='bg-background-80 outline-none border-none ring-0 focus:ring-0'
          
          />
          <Button className='bg-gradient-to-tr w-full from-[#3352cc] font-medium text-white to-[#1c2d70]'>
